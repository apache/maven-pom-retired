...
{     
    ...
    {    
        ...
        String[] editedColumnValues = { SCHEDULE_NAME_EDIT + "modified", SCHEDULE_DESCRIPTION + "updated", "9", cronSchedule, "8" };
        
        assertTrue( "Can not edit schedule" , 
                    getSelenium().isElementPresent( XPathExpressionUtil.matchTableRowOrderedValues( editedColumnValues ) ) );
                                
        // check if the active state has been saved
        clickLinkWithXPath( XPathExpressionUtil.columnElementWithSiblingColumnValues( XPathExpressionUtil.ANCHOR, 
                                                                                      5, "Edit", editedColumnValues) );
    ...
    public void testScheduleAddEditPageInputValidation()
    {
        ...
        assertFalse( "Description field improperly validated", getSelenium().isElementPresent( "//tr/td[span='schedule.name.required']" ) );
    ...
    {
        inputSchedule( scheduleName, scheduleDescription, second, minute, hour, dayOfMonth, month,
